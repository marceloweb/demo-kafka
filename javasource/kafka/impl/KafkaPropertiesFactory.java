	...
	private static void setSSLParameters(Properties props, Server server, IContext context) 
		...
		try {
			...
			if (trustStore != null && trustStore.getHasContents()) {
				...
				IOUtils.copy(Core.getFileDocumentContent(context, trustStore.getMendixObject()), 
						new FileOutputStream(tmpTrustStoreFile));
		...
		try {
			...
			if (keyStore != null && keyStore.getHasContents()) {
				...
				IOUtils.copy(Core.getFileDocumentContent(context, keyStore.getMendixObject()), 
						new FileOutputStream(tmpKeyStoreFile));
